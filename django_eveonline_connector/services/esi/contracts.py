...
def get_eve_character_contracts(character_id, contract_ids_to_ignore=[]):
    ...
    logger.debug("Resolved ID %s to character %s" % (character_id, token.character.name))
    # resolve and clean asset list
    contracts_response = EveClient.call('get_characters_character_id_contracts', token, character_id=character_id)
    logger.debug("Response: %s" % contracts_response[0])
    logger.debug("Obtaining IDs found in contracts of %s" % token.character.name)
    ids_to_resolve = set()
    ...
    contracts = []
    logger.info("Building contract list of %s from ESI response" % token.character.name)
    for contract in contracts_response:
        ...
        items = get_contract_items(character_id, contract['contract_id'])
        logger.debug(items)
...
def get_contract_items(character_id, contract_id):
    logger.info("Getting contract items for for %s" % contract_id)
    token = EveToken.objects.get(evecharacter__external_id=character_id)

    contracts_response = EveClient.call('get_characters_character_id_contracts_contract_id_items', token, character_id=character_id, contract_id=contract_id)
